...
func runBuild(cmd *command) (err error) {
	...
	if !nmpkgs["github.com/SpiderOak/mobile/app"] {
		return fmt.Errorf(`%s does not import "github.com/SpiderOak/mobile/app"`, pkg.ImportPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/SpiderOak/mobile/app": true}, nil
	}
	r, w := io.Pipe()
...
func importsApp(pkg *build.Package) error {
	...
	for _, path := range pkg.Imports {
		if path == "github.com/SpiderOak/mobile/app" {
			return nil
		}
	}
	return fmt.Errorf(`%s does not import "github.com/SpiderOak/mobile/app"`, pkg.ImportPath)
